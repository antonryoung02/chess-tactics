...
import { PIECE_VALUES } from "../tactics/utils.js";
import { colorToPlay } from "./utils.js";
...
export class ChessHelper {
    ...
    materialAdvantageAfterTradesAtSquare(
        ...
        while (i < defenders.length && i < attackers.length) {
            if (advantage < 0) {
                if (prevAdv > prevAdv - attackers[i].value + defenders[i].value) {
                    return prevAdv;
                }
                return advantage;
            }
            prevAdv = advantage;
            advantage -= attackers[i].value;
            if (advantage > 0) {
